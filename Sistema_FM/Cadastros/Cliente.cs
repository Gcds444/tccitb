...
namespace GUI
{
    public partial class Cliente : Form
    {
        ...
        private void Cadastrar_Click(object sender, EventArgs e)
        {
            ...
            if (senha == ""||nome == "" || razao == "" || email == "" || tel == "(  )    -" || CPF == "   ,   ,   -" || UF == "" || Numero == "" || NomeRepresentate == "" || TelRepresentante == "(  )    -" || emailRepresentante == "")
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    r = cliente.SelecionarClienteCPF(txtCPF.Text);
                    o = cliente.SelecionarClienteCNPJ(txtCNPJ.Text);
                    p = cliente.SelecionarClienteEmail(txtEmail.Text);
                    m = cliente.SelecionarClienteEmailRepresentante(txtEmailRepresentante.Text);
                    c = cliente.SelecionarClienteRazao(txtRazao.Text);
                    ...
                    else if (o.Rows.Count > 0)
                    {
                        MessageBox.Show("CNPJ já cadastrado!");
                    }
                    else if (p.Rows.Count > 0)
                    {
                        MessageBox.Show("Email da empresa já cadastrado!");
                    }
                    else if (m.Rows.Count > 0)
                    {
                        MessageBox.Show("Email do representante já cadastrado!");
                    }
                    else if (c.Rows.Count > 0)
                    {
                        MessageBox.Show("Razao já cadastrado!");
                    }
                    else
                    {
                        txtCPF.TextMaskFormat = MaskFormat.ExcludePromptAndLiterals;
                        ...
                        cliente = new BLL.BLLClienteSql();
                        cliente.Inserir(txtNome.Text, txtRazao.Text, txtCNPJ.Text, txtEmail.Text, txtTel.Text, txtCEP.Text, txtLogradouro.Text, txtNumero.Text, txtComplemento.Text, txtBairro.Text, txtCidade.Text, txtUF.Text, txtNomeRepresentante.Text, txtCPF.Text, txtTelRepresentante.Text, txtEmailRepresentante.Text,txtSenha.Text);
                        MessageBox.Show("Dados Inseridos");
